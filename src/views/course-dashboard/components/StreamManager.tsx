...
import { useStreaming } from '../../../context/StreamingProvider';
import { useAuth } from '../../../context/AuthProvider';
import { StreamSetupForm } from './streams/StreamSetupForm';
import { StreamHeader } from './streams/StreamHeader';
import { StreamVideo } from './streams/StreamVideo';
import { StreamControls } from './streams/StreamControls';
...
const StreamManager = () => {
  ...
  const handleStartStream = (details: StreamDetails) => {
  ...
  if (showSetupForm || !isBroadcasting) {
    return <StreamSetupForm onStartStream={handleStartStream} />;
  }
  ...
  return (
    <div className="h-full flex flex-col gap-6 md:p-6 bg-slate-950/90 text-slate-200 rounded-2xl border border-slate-800/50">
      <StreamHeader
        ...
        onGoLiveClick={() => setShowSetupForm(true)}
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className={`${isChatOpen ? 'lg:col-span-2' : 'lg:col-span-3'} flex flex-col gap-6`}>
          <StreamVideo
            ...
            cameraBubbleRef={cameraBubbleRef}
            videoActive={videoActive}
            isSharingScreen={isSharingScreen}
            shouldShowRemoteVideo={shouldShowRemoteVideo}
            getAvatar={getAvatar}
          ...
          <StreamControls
            ...
            toggleVideo={toggleVideo}
            toggleScreenShare={handleToggleScreenShare}
            startStream={() => setShowSetupForm(true)}
