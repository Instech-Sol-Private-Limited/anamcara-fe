        ...
        <div className="w-full min-h-[calc(100vh-100px)] p-2 sm:p-4 md:p-8" style={{ backgroundColor: '#0A0E1A' }}>
            <div className="mb-4 sm:mb-6 md:mb-8 flex flex-col sm:flex-row items-center gap-2 sm:gap-4">
                ...
                <div className="w-12 h-12 sm:w-16 sm:h-16 md:w-20 md:h-20 relative flex-shrink-0">
                    ...
                    <div
                        className="absolute inset-[2px] sm:inset-[3px] md:inset-[4px] bg-[#0A0E1A] flex items-center justify-center overflow-hidden"
                        style={{
                            clipPath: 'polygon(50% 0%, 100% 25%, 100% 75%, 50% 100%, 0% 75%, 0% 25%)',
                        }}
                    >
                ...
                <div className="text-center sm:text-left flex-1">
                    <h1 className="text-lg sm:text-2xl md:text-3xl lg:text-4xl xl:text-5xl font-bold font-mowaq text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-blue-500 to-purple-600 tracking-wider">
                        MANAGE USERS
                    </h1>
                ...
                <>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-3 sm:gap-4 md:gap-6">
                        {users.map((user) => (
                            <div
                                key={user.id}
                                className="relative group transition-all duration-500 ease-out hover:-translate-y-1 sm:hover:-translate-y-2"
                                style={{
                            ...
                            >
                                ...
                                {/* Inner card content */}
                                <div
                                    className="relative p-3 sm:p-4 md:p-6 h-full"
                                    style={{
                                        background: 'linear-gradient(135deg, rgba(26, 26, 46, 0.95) 0%, rgba(22, 33, 62, 0.95) 50%, rgba(15, 23, 42, 0.95) 100%)',
                                        clipPath: 'polygon(2px 2px, calc(100% - 10px) 2px, calc(100% - 2px) 10px, calc(100% - 2px) calc(100% - 2px), 10px calc(100% - 2px), 2px calc(100% - 10px))',
                                    }}
                                >
                                    {/* Action Menu */}
                                    <div className="absolute top-2 right-2 sm:top-3 sm:right-3 md:top-4 md:right-4 z-20">
                                        ...
                                        >
                                            <Button
                                                shape="circle"
                                                size="small"
                                                icon={<MoreOutlined className="text-sm sm:text-base md:text-lg text-gray-400 group-hover:text-cyan-400 transition-colors" />}
                                                className="!bg-black/30 hover:!bg-cyan-500/20 !border-cyan-500/50 backdrop-blur-sm !w-6 !h-6 sm:!w-8 sm:!h-8 md:!w-10 md:!h-10"
        ...
        <div className="w-full h-full rounded-full bg-gradient-to-br from-slate-600 to-slate-800 flex items-center justify-center">
          <span className="text-white font-bold text-lg sm:text-xl md:text-2xl">
            {user.first_name?.charAt(0)?.toUpperCase() || 'U'}
                                        ...
                                        {/* User Name */}
                                        <h3
                                            className="text-sm sm:text-base md:text-lg lg:text-xl font-bold text-white capitalize truncate w-full mb-1 sm:mb-2"
                                        ...
                                        {/* Email */}
                                        <p
                                            className="text-xs sm:text-sm text-cyan-400 mb-2 sm:mb-3 md:mb-4 truncate w-full font-mono"
                                            title={user.email}
                                    ...
                                    {/* Stats Section */}
                                    <div className="mt-auto pt-2 sm:pt-3 md:pt-4 space-y-2 sm:space-y-3">
                                        {/* Status Row */}
                                        <div className="flex justify-between items-center">
                                            <span className="text-gray-400 font-mono text-[10px] sm:text-xs uppercase tracking-wider">Status:</span>
                                            <div
                                                className={`px-2 sm:px-3 py-1 text-[10px] sm:text-xs font-bold uppercase tracking-wider ${user.is_active
                                        ...
                                        <div
                                            className="h-px bg-gradient-to-r from-transparent via-cyan-500 to-transparent mt-2 sm:mt-3 md:mt-4"
                                            style={{ boxShadow: '0 0 6px rgba(0, 220, 255, 0.5)' }}
                                        />
                                    </div>
                                </div>
                            </div>
                    ...
                    {totalUsers > pageSize && (
                        <div className="mt-6 sm:mt-8 md:mt-12 flex justify-center pb-2 sm:pb-4">
                            <Pagination
                                current={currentPage}
                                pageSize={pageSize}
                                total={totalUsers}
